...
locals {
  ssl_cert_listener_indexes = var.extra_ssl_certs.*.https_listener_index
  ssl_cert_arns = var.extra_ssl_certs.*.certificate_arn
}

resource "aws_lb_listener_certificate" "https_listener" {
  count           = var.create_alb && var.logging_enabled ? var.extra_ssl_certs_count : 0
  listener_arn    = aws_lb_listener.frontend_https[local.ssl_cert_listener_indexes[count.index]].arn
  certificate_arn = local.ssl_cert_arns[count.index]
}
